...
public class ChampionnatService {
	...
	/**
	 * Récupère la liste des championnats (ID + nom) par rapport aux critères en entrée
	 * URL en /championnat/
	 * @param niveau
	 * @param annee
	 * @param sexe
	...
	public List<Poule> consulterPoules(int idChampionnat, int idDivision) {
		...
		try {
			httpClient.execute(get1); // Permet de setter le cookie qui sert à se souvenir de l'ID de championnat (la fftest statefull)
