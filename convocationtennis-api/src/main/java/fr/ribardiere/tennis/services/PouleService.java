...
public class PouleService {
	...
	public List<Equipe> consulterEquipe(int idPoule) {
		...
		for (Element elem : elems) {
			...
			if (elems2.size() > 0) { // Permet de ne pas lire la ligne d'entête qui ne contient que des th
	...
	private List<Rencontre> recupererRencontresDansPage(String reponse, int page) {
		...
		Elements trs = doc.select("#tabs"+page+"tab2>table>tbody>tr>td>table>tbody>tr>td>table>tbody>tr>td>table>tbody>tr>td>table>tbody>tr");
	 ...
	 * @param idEquipe
	 * @param journee
	 * @param dateReelle la date réelle de la rencontre (jour + heure de début)
	 * @param isReport
	 * @param motifReport
	 * @return
	 */
	public Convocation recupererDonneesConvocation(int idPoule, int idEquipe, int journee, Date dateReelle, boolean isReport, String motifReport) {
